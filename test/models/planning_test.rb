...
class PlanningTest < ActiveSupport::TestCase
        ...
        [
          ...
          (
            if options[:global] && i == 0
              unassigned_stop_visits.map{ |s| { id: s.id, type: s.optim_type }} +
                all_stop_visits.map{ |s| { id: s.id, type: s.optim_type }}
            elsif !options[:global]
              (i == 0 ? unassigned_stop_visits.select{ |s| s.is_a?(StopVisit) }.map{ |s| { id: s.id, type: s.optim_type }} : []) +
                r.stops.select{ |s| s.is_a?(StopVisit) }.map{ |s| { id: s.id, type: s.optim_type }}
            else
              []
            end +
            r.stops.select{ |s| s.is_a?(StopRest) }.map{ |s| { id: s.id, type: s.optim_type }}
          ).reverse
        ]
      }
    ).to_h
  ...
  test 'shoud set late_multiplier to nil when stop is a geolocalized rest during optimization' do
    planning = plannings(:planning_one)
    uri_template = Addressable::Template.new("#{Planner::Application.config.optimizer.url}/vrp/submit.json")
  ...
  test 'should set stops for one route' do
    ...
    original_order_one = route.stops.map{ |s| { id: s.id, type: s.optim_type }}
    original_order_three = route_three.stops.map{ |s| { id: s.id, type: s.optim_type }}
    route.planning.set_stops([[route.id, original_order_one + original_order_three.reverse]].to_h)
    assert_equal(
      original_order_one + original_order_three.reverse,
      route.planning.routes.find{ |r| r.id == route.id }.stops.map{ |s| { id: s.id, type: s.optim_type }}
    )
    route.planning.save!
    route.reload
    assert_equal(
      original_order_one + original_order_three.reverse,
      route.stops.map{ |s| { id: s.id, type: s.optim_type }}
    )
  end
  ...
  test 'averages should count one-stop route without depots as used vehicle' do
    ...
    planning.routes.select(&:vehicle_usage?).each do |route|
      ...
      route.route_data.update!(drive_time: nil)
    end

    first_stop = target_route.stops.first
    first_stop.active = true
    first_stop.save!

    # Keep averages payload available while the target route has no drive_time.
    other_route = planning.routes.select(&:vehicle_usage?).find { |route| route.id != target_route.id }
    other_route.route_data.update!(drive_time: 60, distance: 1000) if other_route

    averages = planning.averages('km')
    assert averages
    assert_equal 1, averages[:vehicles_used]
  end
end
