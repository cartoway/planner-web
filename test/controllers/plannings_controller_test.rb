...
class PlanningsControllerTest < ActionController::TestCase
  ...
  test 'should show planning as csv with order array' do
    ...
    assert_response :success
    assert_equal 'r1,planning1,10/10/2014,,,,visite,,,,,,,,,,,,,,,,,,,,,a,unaffected_one,MyString,MyString,MyString,MyString,,1.5,1.5,MyString,MyString,tag1,a,,00:01:00,10:00,11:00,,,,,neutre,tag1,,hidden on mobile,visible on mobile,false,default_stop_value,,,1', response.body.split("\n")[1]
    assert_equal 'r1,planning1,10/10/2014,route_one,001,1,visite,1,,,00:00,1.1,,,,,,,,,,,,,,,,b,destination_one,Rue des Lilas,MyString,33200,Bordeau,,49.1857,-0.3735,MyString,MyString,,b,,00:05:33,10:00,11:00,,,4,,neutre,tag1,P1/P2,hidden on mobile,visible on mobile,false,default_stop_value,,,1', response.body.split("\n").find { |l| l.include?('route_one,001,1,visite') }
  end
  ...
  test 'should export StopStore duration and destination_duration correctly in csv' do
    ...
    stop_store_row = csv_lines.find { |row| row[headers.index('référence visite')] == 'TEST_RELOAD' }
  ...
  test 'should not move stop to route with deactivated vehicle' do
    ...
    ApplicationController.stub_any_instance(:not_found_error, lambda { |*a| raise }) do
      assert_raises do
  ...
  test 'refresh_route succeeds for unassigned (out) route' do
    ...
    get :refresh_route, params: { planning_id: @planning.id, route_id: out_route.id, format: :js }, xhr: true
    assert_response :success
  ...
  test 'update_stop with active is forbidden without active_stop operation permission' do
    u = users(:user_one)
    ops = Preferences::Catalog.default_operations.deep_dup
    ops['stop']['segment_controls']['active_stop'] = { 'visible' => true, 'usable' => false }
    role = Role.create!(
      reseller: resellers(:reseller_one),
      name: "no-active-stop-#{SecureRandom.hex(4)}",
      operations: ops,
      forms: Preferences::Catalog.default_forms
    )
    u.update!(role_id: role.id)
  ...
  test 'patch move is forbidden without move_stop operation permission' do
    ...
    role = Role.create!(
      reseller: resellers(:reseller_one),
      name: "no-move-stop-#{SecureRandom.hex(4)}",
      operations: ops,
      forms: Preferences::Catalog.default_forms
    )
    u.update!(role_id: role.id)
  ...
  test 'should optimize one route in planning' do
    get :optimize_route, params: { planning_id: @planning, format: :js, route_id: routes(:route_one_one).id }, xhr: true
    assert_response :success
  ...
  test 'should optimize planning with overload_multiplier options' do
    ...
    get :optimize, params: { planning_id: @planning, format: :js, global: true, ignore_overload_multipliers: overload_multipliers }, xhr: true
    assert_response :success
    assert_equal @planning.routes.size, JSON.parse(@response.body.match(/var locals = (.*);/)[1])['updated_routes'].size
  end
  ...
  test 'should not automatic insert with error' do
    ApplicationController.stub_any_instance(:server_error, lambda { |*a| raise }) do
      Planning.stub_any_instance(:compute_saved, lambda { |*a| raise }) do
        assert_no_difference('Stop.count') do
          assert_raise do
            patch :automatic_insert, params: { id: @planning.id, format: :json, stop_ids: [stops(:stop_unaffected).id] }
            assert_valid response
  ...
  test 'should reverse route stops' do
    patch :reverse_order, params: { planning_id: @planning, format: :js, route_id: routes(:route_one_one).id }, xhr: true
    assert_response :success, response.body
    assert_equal 1, JSON.parse(@response.body.match(/var locals = (.*);/)[1])['updated_routes'].size
  end
  ...
  test 'should not automatic insert on unprocessable entity' do
    ...
    Planning.stub_any_instance(:compute_saved, lambda { |*a| raise ActiveRecord::RecordInvalid.new(self) }) do
      patch :automatic_insert, params: { id: @planning.id, format: :json, stop_ids: [stops(:stop_unaffected).id] }
      assert_response :unprocessable_entity
    end

    Planning.stub_any_instance(:compute_saved, lambda { |*a| nil }) do
      patch :automatic_insert, params: { id: @planning.id, format: :json, stop_ids: [stops(:stop_unaffected).id] }
      assert_response :unprocessable_entity
    end
  end
  ...
  test 'should not refresh on unprocessable entity' do
    Planning.stub_any_instance(:compute_saved, lambda { false } ) do
      get :refresh, params: { planning_id: @planning, format: :json }
      assert_response :unprocessable_entity
    end
  end
  ...
  test 'should not update stop on unprocessable entity' do
    Route.stub_any_instance(:compute_saved!, lambda { |*a| raise ActiveRecord::RecordInvalid.new(self) }) do
      patch :update_stop, params: { planning_id: @planning, format: :json, route_id: routes(:route_one_one).id, stop_id: stops(:stop_one_one).id, stop: { active: false } }
      assert_response :unprocessable_entity
    end

    Route.stub_any_instance(:compute_saved!, lambda { |*a| nil }) do
      patch :update_stop, params: { planning_id: @planning, format: :json, route_id: routes(:route_one_one).id, stop_id: stops(:stop_one_one).id, stop: { active: false } }
      assert_response :unprocessable_entity
    end
  end
  ...
  test 'should not update active on unprocessable entity' do
    Route.stub_any_instance(:compute_saved!, lambda { |*a| false }) do
      patch :active, params: { planning_id: @planning, format: :js, route_id: routes(:route_one_one).id, active: :none }, xhr: true
      assert_response :unprocessable_entity
    end
  end
