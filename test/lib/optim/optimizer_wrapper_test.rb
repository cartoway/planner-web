...
class OptimizerWrapperTest < ActionController::TestCase
  ...
  test 'should optimize' do
    begin
      OptimizerWrapper.stub_any_instance(:build_vrp, JSON.parse(File.new(File.expand_path('../../', __dir__) + '/fixtures/optimizer-wrapper/submitted-vrp.json').read, symbolize_names: true)) do
        ...
        assert_equal(
          {1 => [{type: "service", id: 1}, {type: "service", id: 2}, {type: "service", id: 4}, {type: "rest", id: 5}]},
          @optim.optimize(@planning, routes, **{ optimize_minimal_time: 3 })
        )
      end
    ensure
  ...
  test 'should reject regulatory rests with different duration lapse ratios' do
    begin
      ...
      enable_regulatory_rest!(vehicle_usage, duration: 45.minutes.to_i, lapse: 6.hours.to_i)
      other.reload.update!(rest_duration: 30.minutes.to_i)
