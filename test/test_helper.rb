...
def delete_job(part, param = {})
  part = part ? '/' + part.to_s : ''
  delete "/api/0.1/jobs#{part}.json?api_key=testkey1&" + param.collect{ |k, v| "#{k}=" + Adressable::URI.escape(v.to_s) }.join('&')
end

def get_jobs(part, param = {})
  get "/api/0.1/jobs#{part}.json?api_key=testkey1&" + param.collect{ |k, v| "#{k}=" + Adressable::URI.escape(v.to_s) }.join('&')
end
