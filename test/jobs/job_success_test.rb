...
class JobSuccessTest < ActiveSupport::TestCase
  ...
  test 'enqueuing inside a locked customer transaction records queued without deadlocking' do
    customer = customers(:customer_one)
    planning = plannings(:planning_one)
    customer.update!(last_async_jobs: {})
    ...
    Timeout.timeout(5) do
      Customer.transaction do
        Customer.where(id: customer.id).update_all(updated_at: Time.current)
        delayed_job = Delayed::Job.enqueue(OptimizerJob.new(customer.id, planning.id, nil, {}))
        assert_equal 'queued', customer.reload.last_async_jobs.dig('optimizer', 'status')
        assert_equal delayed_job.id, customer.last_async_jobs.dig('optimizer', 'id')
      end
    end
  end
