...
class ImporterDestinationsTest < ActionController::TestCase
  ...
  test 'should import many-utf-8 in new planning' do
    ...
    planning = @customer.plannings.create!(name: 'plan été', vehicle_usage_set: vehicle_usage_sets(:vehicle_usage_set_one), tags: [@customer.tags.build(label: 'été')])
  ...
  test 'should import one plan and update same plan with stores' do
    ...
    assert_difference('Planning.count', 0) do
      assert_difference('Route.count', 0) do
        assert_difference('Stop.count', 0) do # +1 visit -1 store
          # Put visit v1 in out_route and introduce new visit v10
          assert ImportCsv.new(importer: ImporterDestinations.new(@customer), replace: false, file: tempfile('test/fixtures/files/import_destinations_single_plan_one_route_v1_out_route_v2.csv', 'text.csv')).import
          @customer.reload
          planning = @customer.plannings.last
  ...
  test 'should import one plan with rest stop at specified index' do
    Planning.all.each(&:destroy)
    @customer.delete_all_destinations
    @customer.vehicle_usage_sets.each{ |vus| vus.vehicle_usages.each{ |vu| (vu.active = true) && vu.save }}
    @customer.reload
    ...
    assert_difference('Planning.count', 1) do
      assert_difference('StopVisit.count', 2) do
        assert_difference('StopRest.count', 2) do
          ...
          assert_equal 1, route_1.stops.find { |stop| stop.visit&.ref == 'v1' }.index
          assert_equal 2, route_1.stops.find { |stop| stop.is_a?(StopRest) }.index
          assert_equal 3, route_1.stops.find { |stop| stop.visit&.ref == 'v2' }.index
  ...
  test 'should import new visits with existing tag to existing planning via JSON without stale update' do
        ...
        visits: [
          {
            ...
            tag_ids: [visit_tag.id]
          }
        ]
      }
    ]

    # Import via JSON - planning is passed to ImporterDestinations constructor
    ...
    # Should import without stale update error
    ...
    assert_equal initial_visit_count + 2, final_visit_count, 'New visits should be added to planning'

    # Verify visits have the existing tag
    new_visits = existing_planning.visits.select{ |v| ['new_visit_1', 'new_visit_2'].include?(v.ref) }
    assert_equal 2, new_visits.size, 'Both new visits should be in planning'
    new_visits.each do |visit|
      assert visit.tags.include?(visit_tag), "Visit #{visit.ref} should have the existing tag #{visit_tag.label}"
    end
