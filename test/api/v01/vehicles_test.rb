...
class V01::VehiclesTest < ActiveSupport::TestCase
  ...
  def api(part = nil, param = {})
    part = part ? '/' + part.to_s : ''
    "/api/0.1/vehicles#{part}.json?api_key=testkey1&" + param.collect{ |k, v| "#{k}=" + Adressable::URI.escape(v.to_s) }.join('&')
  end

  def api_admin(part = nil, param = {})
    part = part ? '/' + part.to_s : ''
    "/api/0.1/vehicles#{part}.json?api_key=adminkey&" + param.collect{ |k, v| "#{k}=" + Adressable::URI.escape(v.to_s) }.join('&')
  end

  def api_deliverables(id, param = {})
    "/api/0.1/vehicles/#{id}/deliverable_units?api_key=testkey1&" + param.collect{ |k, v| "#{k}=" + Adressable::URI.escape(v.to_s) }.join('&')
  end
