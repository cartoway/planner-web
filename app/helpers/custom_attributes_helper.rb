module CustomAttributesHelper
  def custom_attribute_default_value_form_field(object_type, typed_default_value)
    ...
    when 'integer'
      number_field_tag 'custom_attribute[default_value]', typed_default_value, class: 'form-control'
    when 'float'
  ...
  def object_type_cast(object_type, value)
    case object_type
    when 'boolean'
      json_array?(value) ? ActiveRecord::Type::Boolean.new.cast(value) : false
    when 'integer'
    ...
    when 'array'
      ...
      json_array?(value) ? JSON.parse(value).first : ""
    end
  end
