...
* `Destinations` describes the geographical points or entity. `Visits` holds the actions to be performed and the constraints associated. `Visits` which might be directly nested in the destination definition. The same `Destination` can be visited several times. An `Visit` might have multiple quantities and should be linked to a `DeliverableUnit`.
