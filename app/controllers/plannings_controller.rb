      ...
      routes_with_configured_devices =
        if key == :deliver
          @planning.routes.select do |route|
            route.vehicle_usage_id && route.vehicle_usage.vehicle.contact_email.present?
          end
        else
          @planning.routes.select do |route|
