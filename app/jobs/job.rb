...
class Job < Struct
  ...
  def remember_async_job!(delayed_job, status)
    ...
    # queued/working must be visible while the worker transaction is still open.
    if %w[queued working].include?(status)
      Thread.new(&record).join
    else
      record.call
      delayed_job.async_outcome_remembered = true if delayed_job.respond_to?(:async_outcome_remembered=)
    end
  end
