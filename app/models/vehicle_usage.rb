...
class VehicleUsage < ApplicationRecord
  ...
  def default_store_rest
    return if regulatory_rest_intent?
  ...
  def default_rest_lapse
    return unless regulatory_rest_intent?

    rest_lapse.to_i.positive? ? rest_lapse : vehicle_usage_set.rest_lapse
  end
  ...
  def outside_default_work_time?(start_time, current_time, rests_duration: 0)
    return false unless default_work_time

    work_elapsed = current_time.to_i - start_time.to_i - rests_duration.to_i
    work_elapsed > default_work_time
  end
  ...
  def work_time_limit_seconds
    default_work_time || (default_time_window_end.to_i - default_time_window_start.to_i)
  end

  def work_or_window_time
    hour_value = ChronicDuration.output(work_time_limit_seconds, limit_to_hours: true, format: :chrono, units: 5)
  ...
  def rest_stop_after_rest_start
    if self.rest_stop.present? && ((self.rest_start.present? && self.rest_stop < self.rest_start) || self.default_rest_start.blank?)
  ...
  def normalize_rest_type
    # Rest type is chosen on the vehicle usage set, not on the usage.
    self.rest_mode = regulatory_rest_intent? ? 'regulatory' : 'window'
    if rest_mode == 'regulatory'
      self.rest_start = nil
      self.rest_stop = nil
  ...
  def regulatory_rest_intent?
    vehicle_usage_set&.rest_lapse.to_i.positive?
  end
