...
export const initRestTypeFields = function(prefix) {
  ...
  var currentRestType = function() {
    var $checked = $(restMode + ':checked');
    if ($checked.length) return $checked.val();
    return $(restMode + '[type="hidden"]').val() || 'window';
  };

  var applyMode = function(userChanged) {
    var regulatory = currentRestType() === 'regulatory';
  ...
  $form.on('submit', function(e) {
    if (currentRestType() !== 'regulatory') return true;
