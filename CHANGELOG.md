...
## V111.0.0
  ### Added
  - API-web: sign in with header `Api-Key` (same as REST) or a short-lived `embed_token` (`POST /api/0.1/embed_tokens`; query, `Embed-Token`, or `Authorization: Bearer`). Optional `origin` sets CSP `frame-ancestors`.
